...
class Error:
	...
	def as_string(self):
		...
		result += f'Archivo {self.pos_start.fn}, linea {self.pos_start.ln + 1}'
...
class RTError(Error):
	...
	def generate_traceback(self):
		...
		return 'Rastreo (llamada más reciente):\n' + result
...
#constantes para diferentes tokens  ----  TT = Token type
TT_INT = 'ENTERO'
TT_STRING = 'CARACTER'
TT_FLOAT = 'FLOTANTE'
...
TT_LPAREN = 'IPAREN'
TT_RPAREN = 'DPAREN'
TT_LSQUARE = 'ICORCHETE'
TT_RSQUARE = 'DCORCHETE'
TT_EE = 'IGUALIGUAL'
TT_NE = 'NOIGUAL'
TT_LT = 'MENORQUE'
TT_GT = 'MAYORQUE'
TT_LTE = 'MENORIGUALQUE'
...
#función repr para que luzca mejor a la hora de imprimir en terminal	
	def __repr__(self):
...
#tomaremos el texto que estamos procesando, tendremos rastreada la posición actual y el caracter actual
class Lexer:
...
#ignoraremos espacios y tabs, revisara si el caracter es un +,-,*,/,(,) más digitos
	...
	def make_not_equals(self):
		...
		self.advance()
		return None, ExpectedCharError(pos_start, self.pos, "'=' (después '!')")
...
#Como cataloga cada caracter para realizar las operaciones de la multiplicación
#BinOp -> es la operación binaria que se ralizara con un NodoIzquierdo, el operador, y el Nodo Derecho
#  8 * 9 = 8(NodoIzquierdo) *(OperaciónBinaria) 9(Nodo Derecho)
...
class Parser:
		...
		if self.current_tok.type == TT_LPAREN:
			...
			if self.current_tok.type == TT_RPAREN:
				...
				if res.error:
					return res.failure(InvalidSyntaxError(
						self.current_tok.pos_start, self.current_tok.pos_end,
						"Experando ENTERO, FLOTANTE, IDENTIFICADOR, '+', '-', '(', 'SI', 'PARA', 'MIENTRAS', 'FUN'"
					))
	...
	def expr(self):
		...
		if res.error:
			return res.failure(InvalidSyntaxError(
				self.current_tok.pos_start, self.current_tok.pos_end,
				"'+', '-' o '(' Esperando ENTERO, FLOTANTE, SI, PARA, MIENTRAS, FUN IDENTIFICADOR"
			))
...
class Value:
	...
	def copy(self):
		raise Exception('Metodo copy no definido')
...
class BaseFunction(Value):
	...
	def check_args(self, arg_names, args):
		...
		if len(args) < len(arg_names):
			return res.failure(RTError(
				self.pos_start, self.pos_end,
				f"{len(arg_names) - len(args)} muy pocos argumentos pasados a '{self.name}'",
				self.context
			))
...
class BuiltInFunction(BaseFunction):
	...
	def no_visit_method(self, node, context):
		raise Exception('No execute_{self.name} method defined')
...
#Metodo Print (Imprimir)
	def execute_print(self, exec_ctx):
		print(str(exec_ctx.symbol_table.get('value')))
		return RTResult().success(Number.null)
	execute_print.arg_names = ['value']
  
#Metodo Ejecutar Print
	def execute_print_ret(self, exec_ctx):
		return RTResult().success(String(str(exec_ctx.symbol_table.get('value'))))
	execute_print_ret.arg_names = ['value']

#Metodo Ejecutar entrada 
	def execute_input(self, exec_ctx):
		text = input()
		return RTResult().success(String(text))
	execute_input.arg_names = []

#Metodo Ejecutar Entrada Entero
	def execute_input_int(self, exec_ctx):
		while True:
			...
			try:
				...
				print(f"'{text}' DEBE SER UN ENTERO, PUEBA DE NUEVO")
		return RTResult().success(Number(number))
	execute_input_int.arg_names = []

#Metodo limiar Pantalla
...
#Funcion añadir elento a la lista
...
#Funcion eliminar elemnto de lista
	def execute_pop(self, exec_ctx):
		...
		if not isinstance(index, Number):
			return RTResult().failure(RTError(
				self.pos_start, self.pos_end,
				"El segundo valor debe ser la posicion del elemento en numero entero",
				exec_ctx
			))
...
#el interprete tendra un metodo visit que procesara node y luego visitará todos los child nodes
...
class Interpreter:
	...
	def visit_VarAccessNode(self, node, context):
		...
		value = value.copy().set_pos(node.pos_start, node.pos_end).set_context(context).set_context(context)
		return res.success(value)
